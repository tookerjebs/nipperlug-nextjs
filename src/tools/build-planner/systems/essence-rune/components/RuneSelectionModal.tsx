/**
 * RuneSelectionModal - Modal for selecting essence runes
 * Adapted from StatSelectionModal for essence rune system
 * 
 * DUPLICATION NOTICE:
 * This file is nearly identical to the corresponding file in the karma-rune system.
 * If making changes here, consider whether the same changes should be applied to:
 * src/tools/build-planner/systems/karma-rune/components/RuneSelectionModal.tsx
 */
...
interface RuneSelectionModalProps {
  ...
  onClose: () => void;
  slotNumber: number;
  availableRunes: EssenceRune[];
  onRuneSelect: (slotNumber: number, rune: EssenceRune) => void;
  onRemoveRune?: (slotNumber: number) => void;
  currentRune?: EssenceRune | null;
}
...
const RuneItem = memo(({ rune, onSelect }: { rune: EssenceRune; onSelect: (rune: EssenceRune) => void }) => {
  // Memoize stat info lookup since it's expensive
  const statInfo = useMemo(() => getStatInfo(rune.baseStatType), [rune.baseStatType]);
...
export default function RuneSelectionModal({
  ...
  const handleRuneSelect = useCallback((rune: EssenceRune) => {
    onRuneSelect(slotNumber, rune);
    onClose();
  }, [slotNumber, onRuneSelect, onClose]);
  ...
  const runesByCategory = useMemo(() => {
    return filteredRunes.reduce((acc, rune) => {
      ...
      return acc;
    }, {} as Record<string, EssenceRune[]>);
  }, [filteredRunes]);

  // Memoize remove handler to prevent function recreation
  const handleRemove = useCallback(() => {
    if (onRemoveRune) {
      onRemoveRune(slotNumber);
      onClose();
    }
  }, [onRemoveRune, slotNumber, onClose]);
        ...
        <div className="flex items-center justify-between p-4 border-b border-border-dark">
          <div>
            <h2 className="text-xl font-bold text-white">
              Select Essence Rune for Slot {slotNumber}
            </h2>
            {currentRune && (
              <p className="text-sm text-gray-400">
                Current: {currentRune.name}
              </p>
            )}
          </div>
          <button
