...
import { calculateCombatPower } from '../data/cp-weights';
// CharacterClass type removed - using damage types instead
// Class damage utilities no longer needed - using damage types directly
...
export interface DamageStats {
  ...
  selectedClass: 'sword' | 'magic' | null;
...
interface BuildPlannerActions {
  ...
  setSelectedClass: (damageType: 'sword' | 'magic' | null) => void;
...
function recalculateDamageStats(
  ...
  level: number, 
  enemyConfig: EnemyConfig,
  selectedClass: 'sword' | 'magic' | null = null
): DamageStats {
  ...
  const legacyDamageValues = calculateDamage(buildStats, level, enemyConfig, selectedClass);
  ...
  // If damage type is selected, calculate type-specific PvE damage
  if (selectedClass) {
    const damageType = selectedClass; // selectedClass is now the damage type directly
...
export const useBuildPlannerStore = create<BuildPlannerState & BuildPlannerActions>()(subscribeWithSelector((set, get) => ({
  ...
  setSelectedClass: (damageType: 'sword' | 'magic' | null) => {
    set((state) => ({
      selectedClass: damageType,
      damageStats: recalculateDamageStats(state.buildStats, state.characterLevel, state.enemyConfig, damageType),
    }));
  },
...
// Note: Class selection is now handled directly through the damage type system
// No need to subscribe to external class stores
